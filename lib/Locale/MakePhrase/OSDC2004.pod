...
strings) can be a complicated problem.  Existing possible solutions
...
Most translations system are loosly based around one of these concepts.
...
and returns a string based on the language.
...
=head1 How would you or me speak a phrase in a second language?
...
that you speak most often.

=item 2)

They try to understand what it is they are trying to say; they
...
The important point is that the information of the phrase is what is
being translated, and this is determined by all of the various
pieces of information surrounding that phrase.
...
anyone else that can speak more than one language...

The general philesophy is that for phrases that dont have any of the
'fill in the blanks' (as in "Please choose some ___"), then it is a
relatively simple problem of translating the phrase; its generally
...
phrase for all possible value values of blank.

Now, if the blank has a value of zero then we, ideally, would like
...
test are common to all languages?

=over 6

=item Note:

I have just said "non fill in the blanks" is relatively simple, in the
...
=head1 Dont assume a property of one language, is applicable to others.
...
person that is capable of understanding the nuances of every language.
Thus it is pointless to even try to make a property of one language,
also apply to another.

Lets look at an example - Chinese vs Engligh. 
...
Where as Engligh requires two seperate phrases, one for singular
and one for plural.
...
A linguistic rule are the properties that encapsulate the technique
...
Under some circumstances, there may be multiple expression which
evaluate to true.  The priority is used to determine which expression
to evaluate first.

=back

A linguistic rule, from programmers point of view, is a struct which
contains enough information so that it enables us to implement an
...
The linguistuc rules for the fallback languages which match the key,
are also retrieved.
...
Apply the arguments in-place, to the selected rules' translated value.
...
Be able to swap the ordering of the blanks, ie positonal argument 2
...
Lets say you were talking about a person, specifically a female child.
In Italian the term used would be 'bambina'; for a male child it is
'bambino'.  Thus in this case, the context surrounding the phrase will
include the age and gender of the child.
